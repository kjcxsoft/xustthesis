...
% \author{Zeping Lee\thanks \and
%   }
\date{v3.2.1\qquad 2021-03-12}
\maketitle
...
\begin{latex}
  \documentclass[degree-type=professional]{xustthesis}
\end{latex}

\DescribeOption{language}
论文全文的主要语言。可选：\opt{chinese}（默认），\opt{english}。
\begin{latex}
  \documentclass[language=english]{xustthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[output=electronic]{xustthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[section-style=arabic]{xustthesis}
\end{latex}
...
\begin{latex}
  \documentclass[fontset=windows]{xustthesis}
\end{latex}
...
其中的各项信息使用 \cs{xustsetup} 命令的方式填写，如：
\begin{latex}
  \xustsetup{
    title  = {论文中文题目},
...
有几点说明：
\begin{itemize}
  \item \cs{xustsetup} 使用 \pkg{kvsetkeys} 机制，配置项之间不能有空行，否则会报错。
...
\DescribeOption{keywords*}
摘要的关键词应使用 \cs{xustsetup} 的接口进行设置，
只要在摘要结束前即可，比如：
\begin{latex}
  \begin{abstract}
    这里是摘要。
    \xustsetup{keywords = {论文；摘要；关键词}}
  \end{abstract}
\end{latex}
...
算法可以使用 \pkg{algorithms} 宏包，推荐使用较新的 \pkg{algorithm2e}。
...
参考文献表可以使用 \BibTeX{} 生成，并在文中使用 \cs{printbibliography} 命令调用。
...
但是需要注意：
\begin{enumerate}
  \item 在 \file{example.tex} 中调用每章的子文件时必须使用 \cs{include} 命令；
